...
<body>
  ...
  <main class="conf-steps">
          ...
          <div class="conf-step__wrapper">
            ...
            <div class="conf-step__hall">
              <div class="conf-step__hall-wrapper">
                <!-- Схема зала будет загружена здесь через JS -->
              </div>
            </div>

            <fieldset class="conf-step__buttons text-center">
              <button type="button" class="conf-step__button conf-step__button-regular cancel">Отмена</button>
        ...
        <section class="conf-step">
          ...
          <div class="conf-step__wrapper">
            ...
            <ul class="conf-step__selectors-box">
              @foreach ($halls as $hall)
                <li>
                  <input type="radio" class="conf-step__radio" name="chairs-hall1" value="{{ $hall->id }}" {{ $loop->first ? 'checked' : '' }}>
                  <span class="conf-step__selector">{{ $hall->name }}</span>
                </li>
              @endforeach
            </ul>     
            ...
            <p class="conf-step__paragraph">Установите цены для типов кресел:</p>
              <div class="conf-step__legend">
                <label class="conf-step__label">Цена, рублей
                  <input type="text" class="conf-step__input price" placeholder="0">
                </label>
              ...
              <div class="conf-step__legend">
                <label class="conf-step__label">Цена, рублей
                  <input type="text" class="conf-step__input vip_price" placeholder="0">
                </label>
    ...
    @else
      <!-- Сообщение при отсутствии залов -->
      <section class="conf-step">
        <header class="conf-step__header conf-step__header_opened">
          <h2 class="conf-step__title">Конфигурация залов</h2>
        </header>
        <div class="conf-step__wrapper">
          <p class="conf-step__paragraph">Нет доступных залов. Пожалуйста, создайте зал, чтобы продолжить.</p>
          <button class="conf-step__button conf-step__button-accent" onclick="addHall()">Создать зал</button>
        </div>
      </section>
      ...
      <section class="conf-step">
        ...
        <div class="conf-step__wrapper">
          <p class="conf-step__paragraph">Нет доступных залов для настройки цен. Пожалуйста, создайте зал сначала.</p>
        </div>
      </section>
    @endif
    ...
    <section class="conf-step">
      ...
      <div class="conf-step__wrapper">
        ...
        <div class="conf-step__movies">
          <!-- Фильмы будут загружены здесь через JS -->
        </div>

        <form action="/api/seance" id="seance_update" method="post">
          @csrf  
          <div class="conf-step__seances">
            <!-- Сеансы будут загружены здесь через JS -->
          </div>
